import { fetchLprRates, hasTushareToken, FALLBACK_LPR_RATES, PBC_BENCHMARK_RATES, PBC_BENCHMARK_HISTORY } from '../../services/lprService.js';
    ...
    data() {
        return {
            calculator: {
                ...
                benchmarkCustomRate: 4.90, // 自定义基准利率
    ...
    methods: {
        ...
        calculateInterest() {
            ...
            if (calc.rateType === 'lpr' && calc.lprCalcMethod === 'specified') {
                ...
                const lprRecord = this.lprRates.find(r => r.date === targetDate) || this.getLprRecordForDate(targetDate);
            ...
            } else if (calc.rateType === 'segmented' || (calc.rateType === 'lpr' && calc.lprCalcMethod === 'segmented')) {
                // 自动分段 (逐日累加)
                let totalInterest = 0;
                    ...
                    if (useBenchmark) {
                        ...
                        if (calc.benchmarkCalcMethod === 'segmented') {
                            ...
                            if (calc.selectedBenchmarkDate === 'custom') {
                                ...
                                const historyRecord = PBC_BENCHMARK_HISTORY.find(r => r.date === calc.selectedBenchmarkDate);
                                if (historyRecord) {
                                ...
                                <div class="smart-form-group">
                                    ...
                                    <div v-if="calculator.customDurationType === 'dateRange'">
                                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 8px;">
                                            <div>
                                                <input type="date" class="smart-input" v-model="calculator.startDate" style="border: 1px solid #e2e8f0;">
                                            </div>
                                            <div>
                                                <input type="date" class="smart-input" v-model="calculator.endDate" style="border: 1px solid #e2e8f0;">
                                            </div>
                                        </div>
