...
export const faruiCaseService = {
  ...
  formatCaseResults(data) {
    ...
    const formattedResults = caseResult.map((item, index) => {
      ...
      return {
        ...
        courtProvince:
          trialCourt.province && trialCourt.province !== 'null' ? trialCourt.province : null,
        courtCity: trialCourt.city && trialCourt.city !== 'null' ? trialCourt.city : null,
        courtLevel:
          trialCourt.commonLevel && trialCourt.commonLevel !== 'null'
            ? trialCourt.commonLevel
            : null,
  ...
  async saveSearchHistory(searchData) {
    try {
      ...
      const { error } = await supabase.from('case_search_history').insert({
        user_id: user.id,
  ...
  async getUserCaseCache(queryHash, pageNumber, pageSize) {
    try {
      ...
      const { data: cases } = await supabase.from('case_cache').select('*').in('case_id', targetIds) // ✅ 修复: 使用 case_id 而不是 id
   ...
   * 保存用户案例缓存关联
   */
  async saveUserCaseCache({ queryHash, query, caseIds, totalCount, keywords }) {
    try {
      const supabase = getSupabaseClient()
      const {
        data: { user }
      ...
      const { error } = await supabase.from('user_case_cache').upsert(cacheData, {
    ...
    try {
      ...
      if (stored) {
        ...
        if (parsed && parsed.data && parsed.data.length > 0) {
          ...
          // 触发后台更新
          this._fetchAndCacheHistory(limit, user.id).catch(e =>
            console.warn('Background history fetch failed', e)
          )
    ...
    try {
      ...
      if (data) {
        try {
          ...
          // Ignore localStorage quotas or errors
        }
      }
  ...
  async deleteSearchHistory(historyId, query) {
    try {
      ...
      if (query) {
        ...
        await supabase.from('user_case_cache').delete().eq('user_id', user.id).eq('query', query)
      ...
      // 4. 最后删除历史记录条目
      const { error } = await supabase.from('case_search_history').delete().eq('id', historyId)
  ...
  async clearAllSearchHistory() {
    try {
      ...
      const { error } = await supabase.from('case_search_history').delete().eq('user_id', user.id)
...
export const faruiLawService = {
  ...
  async searchLaws(params) {
    const { query, pageNumber = 1, pageSize = 50, filterCondition = {} } = params
  ...
  formatLawResults(data) {
    ...
    const results = lawList.map(item => ({
      ...
      department:
        item.lawDomain?.level1Name || item.lawResultAttributeVo?.promulgationDepartment || '',
