...
export async function syncLprRates(force = false) {
    ...
    if (force || latestDateTimestamp < expectedReleaseDate.getTime()) {
        ...
        try {
            ...
            const startDate = new Date(now.getFullYear(), now.getMonth() - months, 1).toISOString().slice(0, 10).replace(/-/g, '');
            const params = { start_date: startDate };
            const data = await callTushareApi('shibor_lpr', params, 'date,1y,5y');
            ...
            if (data && data.items && data.items.length > 0) {
                ...
                const newRates = data.items.map(item => {
                    const obj = {};
                    fields.forEach((field, index) => {
                        if (field === 'date') obj.date = `${item[index].slice(0, 4)}-${item[index].slice(4, 6)}-${item[index].slice(6, 8)}`;
...
export const FALLBACK_LPR_RATES = [
    ...
    { date: '2023-08-21', '1y': 3.45, '5y': 4.20 },
    { date: '2023-06-20', '1y': 3.55, '5y': 4.20 },
    { date: '2022-08-22', '1y': 3.65, '5y': 4.30 },
    { date: '2022-01-20', '1y': 3.70, '5y': 4.60 },
    { date: '2021-12-20', '1y': 3.80, '5y': 4.65 },
    { date: '2020-04-20', '1y': 3.85, '5y': 4.65 },
    { date: '2019-11-20', '1y': 4.15, '5y': 4.80 },
...
export const PBC_BENCHMARK_HISTORY = [
    { date: '2015-10-24', rates: { '6m': 4.35, '1y': 4.35, '1y-3y': 4.75, '3y-5y': 4.75, 'over5y': 4.90 } },
    { date: '2015-08-26', rates: { '6m': 4.60, '1y': 4.60, '1y-3y': 5.00, '3y-5y': 5.00, 'over5y': 5.15 } },
    { date: '2015-06-28', rates: { '6m': 4.85, '1y': 4.85, '1y-3y': 5.25, '3y-5y': 5.25, 'over5y': 5.40 } },
    { date: '2015-05-11', rates: { '6m': 5.10, '1y': 5.10, '1y-3y': 5.50, '3y-5y': 5.50, 'over5y': 5.65 } },
    { date: '2015-03-01', rates: { '6m': 5.35, '1y': 5.35, '1y-3y': 5.75, '3y-5y': 5.75, 'over5y': 5.90 } },
    { date: '2014-11-22', rates: { '6m': 5.60, '1y': 5.60, '1y-3y': 6.00, '3y-5y': 6.00, 'over5y': 6.15 } },
    { date: '2012-07-06', rates: { '6m': 5.60, '1y': 6.00, '1y-3y': 6.15, '3y-5y': 6.40, 'over5y': 6.55 } },
    { date: '2012-06-08', rates: { '6m': 5.85, '1y': 6.31, '1y-3y': 6.40, '3y-5y': 6.65, 'over5y': 6.80 } },
    { date: '2011-07-07', rates: { '6m': 6.10, '1y': 6.56, '1y-3y': 6.65, '3y-5y': 6.90, 'over5y': 7.05 } }
];
